...
def main():
    ...
    ppo_config = from_dict(
        data_class=AgenticConfig, data=OmegaConf.to_container(cfg, resolve=True), config=Config(cast=[Enum])
    )

    init()
    from roll.pipeline.agentic.agentic_pipeline_knowledge_aug import AgenticPipeline as KnowledgeAugPipeline

    pipeline = KnowledgeAugPipeline(pipeline_config=ppo_config)
