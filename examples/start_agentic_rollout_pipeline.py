...
def main():
    ...
    ppo_config = from_dict(data_class=AgenticConfig, data=OmegaConf.to_container(cfg, resolve=True), config=Config(cast=[Enum]))

    init()
    from roll.pipeline.agentic.agentic_rollout_pipeline import \
        AgenticRolloutPipeline
