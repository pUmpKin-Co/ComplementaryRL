...
class TensorBoardTracker(BaseTracker):
    ...
    def __init__(self, config: dict, **kwargs):
        ...
        self.config = self._sanitize_hparams(config)
        self._safe_add_hparams()
        self.writer.flush()

    @staticmethod
    def _sanitize_hparams(config: dict) -> dict:
        sanitized = {}
    ...
    def _safe_add_hparams(self) -> None:
        try:
            self.writer.add_hparams(hparam_dict=self.config, metric_dict={})
        except TypeError as exc:
            logger.warning(
                f"TensorBoard add_hparams failed with TypeError ({exc}); "
                "falling back to text-based config logging."
            )
            self.writer.add_text("hparams/config", json.dumps(self.config, ensure_ascii=False))
