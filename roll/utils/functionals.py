...
def masked_sum(tensor: torch.Tensor, mask: torch.Tensor, dim: int = None) -> torch.Tensor:
    if dim is not None:
        ...
        return (tensor * mask).sum() if mask.sum() > 0 else torch.tensor(0.0, device=tensor.device)
...
def _flatten_metric_numeric_values(value: Any) -> List[float]:
    ...
    while stack:
        ...
        if isinstance(item, np.ndarray):
            if item.dtype == object:
                stack.extend(list(item.ravel()))
            else:
                flattened.extend(item.astype(np.float64, copy=False).ravel().tolist())
            continue

        if isinstance(item, (np.generic, int, float, bool)):
            try:
...
def _to_python_metric_scalar(value: Any) -> float:
    """Convert a reduced metric to a Python float for tracker compatibility."""
...
def reduce_metrics(metrics: Dict[str, Any], reduce_func: Callable[[Any], Any] = np.mean) -> Dict[str, Any]:
    for key, val in metrics.items():
        if isinstance(val, dict):
            metrics[key] = reduce_metrics(val, reduce_func=reduce_func)
            continue

        try:
            reduced = reduce_func(val)
        except Exception:
            flattened = _flatten_metric_numeric_values(val)
            reduced = reduce_func(np.asarray(flattened, dtype=np.float64)) if flattened else float("nan")

        metrics[key] = _to_python_metric_scalar(reduced)
    return metrics
...
def compute_advantage(
    ...
    response_mask=None,
    whiten_advantages_shift_mean=True,
):
    ...
    if whiten_advantages:
        ...
        advantages = masked_whiten(values=advantages, mask=response_mask, shift_mean=whiten_advantages_shift_mean)
