...
class AgenticRolloutPipeline(BasePipeline):
    ...
    def _print_task_pass_rate(self, batch: DataProto) -> None:
        task_key = None
        for candidate in ("task_idx", "task_id"):
            if candidate in batch.non_tensor_batch:
                task_key = candidate
                break

        if task_key is None:
            logger.info("[TaskSelection] Skip pass-rate print: neither task_idx nor task_id found in batch.")
            return

        episode_scores = batch.non_tensor_batch.get("episode_scores", None)
        if episode_scores is None:
            logger.info("[TaskSelection] Skip pass-rate print: episode_scores not found in batch.")
            return

        success_score = 1.0

        stats = defaultdict(lambda: {"success": 0, "total": 0})
        for raw_task, raw_score in zip(batch.non_tensor_batch[task_key], episode_scores):
            task_idx = str(raw_task)
            score = float(raw_score)
            is_success = abs(score - success_score) < 1e-6
            stats[task_idx]["success"] += int(is_success)
            stats[task_idx]["total"] += 1

        if not stats:
            logger.info("[TaskSelection] No task stats in current batch.")
            return

        rows = []
        total_success = 0
        total_count = 0
        for task_idx, task_stats in sorted(stats.items(), key=lambda kv: kv[0]):
            success_num = task_stats["success"]
            total_num = task_stats["total"]
            pass_rate = 100.0 * success_num / max(total_num, 1)
            total_success += success_num
            total_count += total_num
            rows.append((task_idx, success_num, total_num, pass_rate))

        task_col_width = max(len("task_idx"), max(len(task_idx) for task_idx, _, _, _ in rows))
        header = (
            f"| {'task_idx':<{task_col_width}} | {'success':>7} | {'total':>5} | {'pass_rate':>9} |"
        )
        sep = "-" * len(header)
        table_lines = [sep, header, sep]
        for task_idx, success_num, total_num, pass_rate in rows:
            table_lines.append(
                f"| {task_idx:<{task_col_width}} | {success_num:>7} | {total_num:>5} | {pass_rate:>8.2f}% |"
            )
        table_lines.append(sep)

        overall_pass_rate = 100.0 * total_success / max(total_count, 1)
        logger.info(
            "[TaskSelection] Per-task success rate "
            f"(success defined as episode_score == {success_score:.1f})\n"
            + "\n".join(table_lines)
            + f"\n[TaskSelection] Overall: success={total_success}/{total_count}, pass_rate={overall_pass_rate:.2f}%"
        )
    ...
    def run(self):
        ...
        for global_step in count() if self.pipeline_config.max_steps == -1 else range(self.pipeline_config.max_steps):
            ...
            if global_step % self.pipeline_config.logging_steps == 0:
                self._print_task_pass_rate(batch)
