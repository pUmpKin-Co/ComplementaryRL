...
from roll.pipeline.agentic.eval_memory_metrics import compute_eval_memory_split_metrics
...
class AgenticPipeline(BasePipeline):
    ...
    def run(self):
        ...
        for global_step in range(self.pipeline_config.max_steps):
            ...
            with tps_timer:
                ...
                metrics["time/e2e_time"] = e2e_time_timer.last
    ...
    def val(self, global_step):
        ...
        if self.pipeline_config.eval_memory_split.enable:
            eval_metrics.update(compute_eval_memory_split_metrics(eval_batch))
        ...
        for group_name, group_batch in batch_grouped.items():
            ...
            for (
                batch_traj_group_name,
                batch_traj_group,
            ) in batch_traj_grouped.items():
...
# Group level filter function. Return false to filter out group.
def group_filter_fn(group: List[DataProto]):
