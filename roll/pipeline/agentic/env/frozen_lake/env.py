...
class FrozenLakeEnv(Env, GymFrozenLakeEnv):
    def __init__(
        ...
        special_token_list=("<|im_start|>", "<|im_end|>", "<think>", "<answer>", "</think>", "</answer>"),
        **kwargs,
    ):
        ...
        if memory_path is not None:
            ...
            for memory_item in memory:
                memory_message += f"{memory_item}\n\n"
            self.memory_message = memory_message
        else:
            self.memory_message = None

        GymFrozenLakeEnv.__init__(
            self, desc=random_map, is_slippery=is_slippery, render_mode=self.render_mode, **kwargs
        )
    ...
    def reset(self, seed=None):
        ...
        try:
            with all_seed(seed):
                ...
                GymFrozenLakeEnv.__init__(
                    self, desc=random_map, is_slippery=self.is_slippery, render_mode=self.render_mode
                )
    ...
    @property
    def player_pos(self):
        state = getattr(self, "s", None)
        if state is None:
            # GymFrozenLakeEnv sets self.s during reset; before that, default to start
            return (0, 0)
        return (state // self.ncol, state % self.ncol)  # (row, col)
