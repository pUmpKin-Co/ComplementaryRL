...
from roll.pipeline.agentic.env.rock import rock_setup_config
...
try:
    from rock.sdk.sandbox.request import BashAction, Command, CreateBashSessionRequest
    from rock.sdk.sandbox.response import BashObservation, CommandResponse
except ImportError:
    print("rock-rl is not available, try import from rl-rock(new-version)")
    try:
        from rock.actions import BashAction, BashObservation, Command, CommandResponse, CreateBashSessionRequest
    except ImportError:
        print("rl-rock is still not available.  Make sure it's installed. ")
...
class SandboxManager:
    ...
    def __init__(self,
        ...
        self.core_tools = list(core_tools) if core_tools is not None else None
    ...
    def _initialize_sandbox(self):
        ...
        if self.start_script:
            self.logger.info(f"[AGENT_START] START - Starting start.sh")
            try:
                success = self._run_start_script(self.agent_session_name, self.start_script)
                if success:
                    self.logger.info(f"[AGENT_START] Success! - run start.sh successfully")
                else:
                    self.logger.warning(f"⚠️ [AGENT_START] Warning! - run start.sh failed")
            except Exception as e:
                self.logger.warning(f"[AGENT_START] ERROR! - run start.sh failed, error: {e}")

        # 清理沙盒环境中的测试文件
        self.logger.info(f"[CLEANUP] START - Cleaning up problematic files in sandbox")
        try:
            success, message = self._cleanup_problematic_files(self.agent_session_name)
            if success:
                self.logger.info(f"[CLEANUP] Success! - Problematic files cleaned up successfully")
            else:
...
"""
    ...
    def execute_command(self, session_name: str, command: str, max_retry: int = 3, backoff: float = 10.0, timeout: float = None) -> RunSessionResponse:
        """
        ...
        try:
            ...
            self.logger.error(f"Error executing command in session '{session_name}': {e}, {self.error_suffix}")
            response = RunSessionResponse()
            response.exit_code = -1
    ...
    def _install_agent(self, session_name: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if self.run_region == "sg":
                set_up_install_script = iflow_config.iflow_set_up_install_script_roll_sg
            elif self.dataset_tag == "terminal_bench_v2.0":
                set_up_install_script = iflow_config.iflow_set_up_install_script_roll_bench
            else:
                set_up_install_script = iflow_config.iflow_set_up_install_script_roll_syn
            ...
            run_status, result = self.run_session_with_timeout(
                ...
                self.install_agent_timeout,
                "install_iflow_info.txt"
            )

            if run_status != RunStatus.SUCCESS:
                return False, f"Installation failed: {run_status}, {result}"
    ...
    def _run_start_script(self, session_name: str, start_script: str) -> bool:
        ...
        try:
            ...
            if bg_response.exit_code == 0:
                self.logger.info(f"[RUN_START_SCRIPT] Start script launched in background successfully")
    ...
    def _cleanup_problematic_files(self, session_name: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if check_response.output and "All files cleaned" in check_response.output:
                self.logger.info(f"[CLEANUP] Success! - All problematic files have been cleaned up")
    ...
    def process_model_response(self, response: str, agent_timeout_sec: int, task_id: str, task_name: str = "") -> Tuple[str, float, bool, bool, Dict[str, Any]]:
        ...
        if "<tool_call>" in response and "bash /tests/run-uv-pytest.sh" not in response:
            have_tool_call = True
            try:
                self.logger.info(f"[TOOL_PARSE] START - Parsing tool call from model response for task_name: {task_name}")
                is_parsed, response_info = self.iflow_tool.execute_action(response, self.agent_session_name)
                
                if not is_parsed:
                    error_msg = response_info if isinstance(response_info, str) and response_info else "Could not parse tool call from response"
    ...
    def run_tests(self, test_files: List[str], test_timeout_sec: int = 60, task_name: str = "") -> dict:
        ...
        if tests_dir.exists():
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if self._compress_directory(str(tests_dir), archive_path):
                    ...
                    self.logger.info(f"[TEST_UPLOAD] Success! - Tests folder uploaded and extracted successfully")
                else:
        ...
        if run_tests_script.exists():
            ...
            if not is_success:
                ...
                return False, FailureMode.TEST_FILE_UPLOAD_FAILED, test_output, ""
            self.logger.info(f"[TEST_UPLOAD] Success! - run-tests.sh uploaded successfully")
        else:
            error_msg = f"run-tests.sh not found: {run_tests_script}"
            self.logger.error(error_msg)
            self.failure_mode = FailureMode.TEST_FILE_UPLOAD_FAILED
            self.error_messages.append(f"Run-tests script not found: {error_msg}")
            return False, FailureMode.TEST_FILE_UPLOAD_FAILED, test_output, test_command

        self.logger.info(f"[TEST_SESSION] Success! - All test files uploaded successfully")
    ...
    def run_ground_truth_solution(self, agent_timeout_sec: int, task_id: Union[str, int], task_name: str = "") -> Tuple[str, float, bool, bool, Dict[str, Any]]:
        ...
        try:
            ...
            if run_status == RunStatus.SUCCESS:
                ...
                self.logger.info(f"[GROUND_TRUTH] Success! - Ground truth solution executed successfully")
                is_valid = True
            elif run_status == RunStatus.TIMEOUT:
    ...
    def get_messages_and_tools(self, request_payload: str):
        ...
        """
        self.logger.debug(f"[GET_MESSAGES_TOOLS] START - Processing request payload")
    ...
    def format_response_payload(self, response: str) -> Tuple[str, Dict]:
        """
        Format response payload for ROCK model service.

        Args:
            response: raw text string from the model, like: "I'\''ll list the contents of the current working directory for you.<tool_call><function=list_directory><parameter=path>/root</parameter></function></tool_call>"

        Returns:
            Tuple of (response_payload, terminated) where:
            - response_payload: JSON string formatted for anti_call_llm method, like response_payload
            - info: Dict containing info
        """
        ...
        if has_tool_calls:
            ...
            try:
                is_parsed, parsed_tool_calls = self.iflow_tool.parse_action(response)
    ...
    def start_model_service(self) -> RunSessionResponse:
        """
        Start the ROCK model service.
        ...
        """
        self.logger.info(f"[ROCK_CLIENT] Starting model service with IFLOW_baseUrl={self.iflow_base_url}...")

        response: RunSessionResponse = self.run_script_with_timeout(session_name=self.agent_session_name,
                                                                    script_text=rock_setup_config.model_service_start_script,
                                                                    timeout=self.default_timeout * 10)
        if response.exit_code == 0:
            self.logger.info("[ROCK_CLIENT] Model service started successfully")
            return response
        else:
            self.logger.error(f"[ROCK_CLIENT] Failed to start model service: {response}, {self.error_suffix}")
            return response

    def fetch_agent_request(self, index: int, response_payload: Optional[str] = None, timeout: float = None) -> RunSessionResponse:
        """
        Anti-call LLM to get next request.

        Args:
            response_payload: Optional response to send to the model service
              response_payload =
                        {
                          "usage": {
                            "prompt_tokens": 1,
                          ...
                          "choices": [
                            {
                              ...
                              "message": {
                                "role": "assistant",
                                "content": "I'll list the contents of the current working directory for you.",
                                "tool_calls": [
                                  {
                                    "type": "function",
                                    "function": {
                                      "name": "list_directory",
                                      "arguments": "{\"path\": \"/root/iflow_test\"}"
                                    }
                                  }
                                ]
                              },
        ...
        Returns:
            Next request payload as JSON string
                json_str like:
                request_payload_json_str = {
                                    "model": "vllm",
                                    "messages":
                                    [
                                        {
                                            "role": "system",
                                            "content": "You are iFlow CLI, an interactive CLI agent with a Chinese name of 心流 CLI, specializing in software engineering tasks. "
                                        },
                                        {
                                            "role": "user",
                                            "content":
        ...
        Raises:
            Exception: If anti-call fails
        """
        if response_payload:
            cmd = rock_setup_config.model_service_anti_call_llm_script.format(index=index, response_payload=shlex.quote(response_payload))
        else:
            cmd = rock_setup_config.model_service_anti_call_llm_no_response_script.format(index=index)

        self.logger.debug(f"[ROCK_CLIENT] Anti-calling LLM with response length: {len(response_payload) if response_payload else 0}")

        try:
            response_obj: RunSessionResponse = self.run_script_with_timeout(
                session_name=self.proxy_session_name,
                script_text=cmd,
                timeout=timeout,
                max_retry=1,
            )
            if response_obj.exit_code == 0:
                self.logger.debug(f"[ROCK_CLIENT] Anti-call LLM succeeded, output length: {len(response_obj.output)}")
                return response_obj
            else:
                self.logger.error(f"[ROCK_CLIENT] Anti-call LLM failed: {response_obj.output}, {self.error_suffix}, response_payload: {response_payload}")
                return response_obj
        except Exception as e:
            self.logger.error(f"[ROCK_CLIENT] Anti-call LLM failed: {str(e)}, {self.error_suffix}, response_payload: {response_payload}")
            response_obj = RunSessionResponse()
            response_obj.exit_code = 1
            response_obj.output = str(e)
            response_obj.failure_reason = str(e)
            return response_obj
