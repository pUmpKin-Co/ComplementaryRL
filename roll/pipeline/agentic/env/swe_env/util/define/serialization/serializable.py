...
class SerializableMixin:
    ...
    def __init_subclass__(cls, decode_into_subclasses: bool | None = None, add_variants: bool = True):
    ...
    def to_dict(self, dict_factory: type[dict] = dict, recurse: bool = True, save_dc_types: bool = False) -> dict:
        ...
        """
        return to_dict(self, dict_factory=dict_factory, recurse=recurse, save_dc_types=save_dc_types)
    ...
    def loads_json(
        ...
        **kwargs,
        ...
        return loads_json(cls, s, drop_extra_fields=drop_extra_fields, load_fn=partial(load_fn, **kwargs))
...
def load(
    ...
    if load_fn is None and isinstance(path, Path):
        ...
        raise ValueError(
            "A loading function must be passed, since we got an io stream, and the " "extension can't be retrieved."
        )
...
def to_dict(
    ...
    for f in fields(dc):
        ...
        if is_dataclass(value) and recurse:
            encoded = to_dict(value, dict_factory=dict_factory, recurse=recurse, save_dc_types=save_dc_types)
...
def from_dict(cls: type[DataclassT], d: dict[str, Any], drop_extra_fields: bool | None = None) -> DataclassT:
    ...
    for field in fields(cls) if is_dataclass(cls) else []:
        ...
        if name not in obj_dict:
            if field.metadata.get("to_dict", True) and field.default is MISSING and field.default_factory is MISSING:
                logger.warning(f"Couldn't find the field '{name}' in the dict with keys " f"{list(d.keys())}")
            continue

        raw_value = obj_dict.pop(name)
        field_value = decode_field(field, raw_value, containing_dataclass=cls, drop_extra_fields=drop_extra_fields)
