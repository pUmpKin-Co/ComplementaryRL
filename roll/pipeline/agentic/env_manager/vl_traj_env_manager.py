...
import roll.pipeline.agentic.env  # 导入环境注册
...
from roll.pipeline.agentic.env_manager.token_mask_utils import split_by_token, token_ids_to_assistant_mask
    ...
    def __init__(
        ...
        **kwargs,
        ...
        with self.thread_lock:
            ...
            self.env = self.wrap_env_with_tools(self.env, self.env_config, kwargs.get("memory_manager"))
    ...
    def make_decision(self, rollout_cache: RolloutCache):
        ...
        for message in messages:
            if message["role"] == "user":
                ...
                images.extend(
                    [content[i].pop("image_PIL") for i in range(len(content)) if content[i]["type"] == "image"]
                )
    ...
    def format_messages(self, history: List[Dict]):
        ...
        for idx, content in enumerate(history):
            ...
            next_step_content = self.next_step_template.format(
                actions_left=content["actions_left"], max_response_length=self.env_config["max_tokens_per_step"]
            )
