...
class StepEnvManager(TrajEnvManager):
    ...
    def make_decision(self, rollout_cache: RolloutCache):
        ...
        for history_step, entry in enumerate(memory_history):
            ...
            sar_history.append(
                f"(step:{self.rollout_cache.step - len(memory_history) + history_step + 1}, observation: {observation}, response: {entry.get('llm_response')})"
            )
        ...
        if len(prompt_ids) >= self.pipeline_config.sequence_length:
            self.logger.warning(
                f"sequence_length = {self.pipeline_config.sequence_length} input_ids length = {len(prompt_ids)},"
        ...
        rollout_cache.history[-1]["messages"].append(
