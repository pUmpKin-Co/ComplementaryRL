...
class MemoryActorPipeline(AgenticPipeline):
    ...
    def _train_memory_model(self, train_batch: DataProto, global_step: int) -> Dict[str, Any]:
        metrics, actor_train_metrics_refs = self._start_memory_model_train(train_batch, global_step)
    ...
    def _start_memory_model_train(
        ...
        return metrics, actor_train_metrics_refs
    ...
    def _has_pending_memory_train(self) -> bool:
        return False

    def _should_resume_memory_manager_for_rollout(self) -> bool:
        return True

    def _maybe_finish_pending_memory_train(self, global_step: int, metrics: Dict[str, Any]) -> None:
        return

    def _finalize_pending_memory_train_before_shutdown(self) -> None:
        return

    def _maybe_train_memory_model_from_buffer(self, global_step: int, metrics: Dict[str, Any]) -> None:
        if self._memory_training_buffer is not None and self._warmup_complete:
            ...
            if batch_size > 0 and self._memory_training_buffer.ready(batch_size):
                logger.info(f"Buffer ready with {len(self._memory_training_buffer)} entries, training memory model...")
                buffer_entries = self._memory_training_buffer.pop_batch(batch_size)
                train_batch = self._build_memory_batch_from_buffer_entries(buffer_entries, global_step=global_step)
    ...
    def run(self):
        ...
        for global_step in range(self.pipeline_config.max_steps):
            ...
            metrics = {}
            with tps_timer:
                with Timer(name="e2e_time", logger=None) as e2e_time_timer:
                    ...
                    if not will_train_memory and not self._has_pending_memory_train():
                    ...
                    if flush_ref is not None:
                        ...
                        flush_pending_updates_time = flush_pending_updates_timer.last
                    else:
                        flush_pending_updates_time = 0
                    ...
                    self._maybe_finish_pending_memory_train(global_step, metrics)
                    ...
                    if self.global_memory_manager is not None and self._should_resume_memory_manager_for_rollout():
                        self.global_memory_manager.resume(global_step)
                    ...
                    if grouped_cfg.enable:
                        ...
                        if raw_group is None:
                            if strict:
                                raise KeyError(
                                    f"grouped_actor_pg.group_key={group_key!r} not found in non_tensor_batch"
                                )
                    ...
                    actor_train_metrics_refs: List[ray.ObjectRef] | None = None
                ...
                with Timer(name="train_memory_model", logger=None) as train_memory_model_timer:
                    self._maybe_train_memory_model_from_buffer(global_step, metrics)
                train_memory_model_time = train_memory_model_timer.last

                if actor_train_metrics_refs is not None:
                    metrics.update(self._collect_train_step_metrics(actor_train_metrics_refs))
                ...
                metrics["time/e2e_time"] = e2e_time_timer.last - train_memory_model_time - flush_pending_updates_time
        ...
        self._finalize_pending_memory_train_before_shutdown()
        ray.get(
            [
...
class FullyAsyncMemoryActorPipeline(MemoryActorPipeline):
    def __init__(self, pipeline_config: MemoryActorConfig):
        super().__init__(pipeline_config)
        self._pending_memory_train: Dict[str, Any] | None = None

    def _has_pending_memory_train(self) -> bool:
        return self._pending_memory_train is not None

    def _should_resume_memory_manager_for_rollout(self) -> bool:
        return self._pending_memory_train is None

    def _maybe_finish_pending_memory_train(self, global_step: int, metrics: Dict[str, Any]) -> None:
        if self._pending_memory_train is None:
            metrics["memory/async_train_pending"] = False
            return
        ...
        if remaining_refs:
            metrics["memory/async_train_pending"] = True
            metrics["memory/async_train_launch_step"] = self._pending_memory_train["launch_step"]
            metrics["memory/async_train_ready_refs"] = len(ready_refs)
            return
    ...
    def _finalize_pending_memory_train_before_shutdown(self) -> None:
        ...
        logger.info("Waiting for pending async memory training to finish before shutdown...")
        self._finalize_pending_memory_train(global_step=self.state.step, metrics={})

    def _finalize_pending_memory_train(self, global_step: int, metrics: Dict[str, Any]) -> None:
        if self._pending_memory_train is None:
            return

        pending_memory_train = self._pending_memory_train
        metrics.update(self._collect_train_step_metrics(pending_memory_train["train_metrics_refs"]))
        metrics.update(self._update_memory_model(global_step))
        metrics.update(compute_data_metrics_for_memory(batch=pending_memory_train["train_batch"]))
        self.memory_actor_train.offload_states(blocking=True)
        ...
        metrics["memory/async_train_pending"] = False
        metrics["memory/async_train_finished_step"] = global_step
        metrics["memory/async_train_launch_step"] = pending_memory_train["launch_step"]
        self._pending_memory_train = None

    def _maybe_train_memory_model_from_buffer(self, global_step: int, metrics: Dict[str, Any]) -> None:
        if self._pending_memory_train is not None:
            metrics["memory/async_train_pending"] = True
            metrics["memory/async_train_launch_step"] = self._pending_memory_train["launch_step"]
            return
        ...
        if self._memory_training_buffer is not None and self._warmup_complete:
            ...
            if batch_size > 0 and self._memory_training_buffer.ready(batch_size):
                logger.info(
                    f"Buffer ready with {len(self._memory_training_buffer)} entries, launching async memory train..."
                )
                buffer_entries = self._memory_training_buffer.pop_batch(batch_size)
                train_batch = self._build_memory_batch_from_buffer_entries(buffer_entries, global_step=global_step)
                ...
                if train_batch is not None:
                    ...
                    metrics.update(memory_train_metrics)
                    metrics["memory/async_train_pending"] = True
                    metrics["memory/async_train_launch_step"] = global_step
                    self._pending_memory_train = {
                        "launch_step": global_step,
                        "train_batch": train_batch,
                        "train_metrics_refs": train_metrics_refs,
                    }
                    return
