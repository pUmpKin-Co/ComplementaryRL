...
def dump_rollout_render(
    ...
    with Timer(name="dump", logger=None) as timer:
        try:
            ...
            def _normalize_frames(frame_list: List) -> List[np.ndarray]:
                normalized: List[np.ndarray] = []
                for frame in frame_list:
                    if isinstance(frame, np.ndarray):
                        normalized.append(frame.astype(np.uint8))
                    else:
                        try:
                            normalized.append(np.asarray(frame, dtype=np.uint8))
                        except Exception:
                            logger.warning("Skipping frame that cannot be converted to numpy array.")
                return normalized

            normalized_frames = [_normalize_frames(frame_list) for frame_list in frames]
            ...
            args_list = [
                (
                    os.path.join(
                        local_save_dir,
                        f"{step}",
                        f"{env_id}_{tag}_{episode_score:.1f}.gif",
                    ),
                    frame_list,
                )
                for frame_list, env_id, tag, episode_score in zip(normalized_frames, env_ids, tags, episode_scores)
                if len(frame_list) > 0
            ]
            ...
            rar_file_path = os.path.join(
                "/tmp",
                f'rollout_render_{datetime.now().strftime("%Y%m%d-%H%M%S")}_{step}.zip',
            )
...
def build_state_group(batch: "DataProto") -> "DataProto":
    ...
    for (
        traj_group_id,
        traj_group_batch,
    ) in batch_group_by_traj_group.items():
...
def compute_response_level_rewards(
    batch: "DataProto", pipeline_config: AgenticConfig, is_memory_batch: bool = False
) -> "DataProto":
    if pipeline_config.adv_estimator == "gigpo":
        ...
        if not is_memory_batch:
            episode_rewards: torch.Tensor = grouped_reward_norm(
                scores_to_group,
                reward_normalization=pipeline_config.reward_normalization,
            )
        else:
            episode_rewards: torch.Tensor = grouped_reward_norm(
                scores_to_group,
                reward_normalization=pipeline_config.memory_reward_normalization,
            )
...
def write_data_json(data, path):
    ...
    with open(path, "w", encoding="utf-8") as f:
        info_str = json.dumps(data, ensure_ascii=False, indent=2)
        f.write(info_str)
    ...
    for checker, func in DUMPING_FUNC:
        if checker(path):
            p = multiprocessing.Process(
                target=func,
                args=(path, write_data, columns_config),
                daemon=False,
            )
            p.start()
