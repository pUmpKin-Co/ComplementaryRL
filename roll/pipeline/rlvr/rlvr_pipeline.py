...
import asyncio
import threading
from roll.utils.tracker import GPUTracker
...
class RLVRPipeline(BasePipeline):
    ...
    def __init__(self, pipeline_config: RLVRConfig):
        ...
        for i, domain in enumerate(domain_list):
            ...
            ).remote(pipeline_config=self.pipeline_config, domain_name=domain)
            ray.get(
        ...
        if self.pipeline_config.record_time_profiler:
            self.rollout_gpu_tracker = GPUTracker(
                interval=0.2,  # Higher frequency for rollout tracking
                filename=f"gpu_rollout_usage_{os.environ.get('WORLD_SIZE', '1')}_{os.environ.get('RANK', '0')}.json",
                output_dir=self.pipeline_config.logging_dir
            )
    ...
    def run(self):
        ...
        for global_step in range(self.pipeline_config.max_steps):
            ...
            with tps_timer, Timer(name="step_total", logger=None) as step_total_timer:
                ...
                batch: DataProto = DataProto(
                    meta_info={"global_step": global_step, "collect_non_finished": self.pipeline_config.async_pipeline}
                )
                ...
                with (
                    ...
                    # Start GPU tracking for rollout generation
                    if self.pipeline_config.record_time_profiler:
                        thread = threading.Thread(target=run_async, daemon=True)
                        thread.start()
                        asyncio.run_coroutine_threadsafe(self.rollout_gpu_tracker.start(global_step), loop)
                    ...
                    # find common keys in all domain_batches
                    common_keys = set(domain_batches[list(domain_batches.keys())[0]].batch.keys())
                    for domain_batch in domain_batches.values():
                        common_keys = common_keys.intersection(set(domain_batch.batch.keys()))
                    for key, domain_batch in domain_batches.items():
                        domain_batch.batch = {key: domain_batch.batch[key] for key in common_keys}
                        new_domain_batch = DataProto.from_dict(domain_batch.batch)
                        new_domain_batch.non_tensor_batch = domain_batch.non_tensor_batch
                        new_domain_batch.meta_info = domain_batch.meta_info
                        domain_batches[key] = new_domain_batch

                    reward_durations = {}
                    for key, domain_batch in domain_batches.items():
                        if "reward_duration" in list(domain_batch.meta_info.keys()):
                            reward_durations[key] = domain_batch.meta_info["reward_duration"]
                    ...
                    if self.pipeline_config.record_time_profiler:
                        self.rollout_gpu_tracker.stop(other_statistic=reward_durations)
        ...
        print(f"[DEBUG] Train[complete!][{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}]")
        logger.info("pipeline complete!")
