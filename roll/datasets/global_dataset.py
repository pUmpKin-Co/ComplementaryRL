...
@ray.remote
class GlobalDataset:
    def __init__(self, dataset_name, split: str = "train", mode="sample", dataset_kwargs: Dict = {}):
        self.mode = mode
        self.dataset_name = dataset_name
        self.split = split
        self.dataset_kwargs = dataset_kwargs
        self.train_ratio = dataset_kwargs.pop("train_ratio", None)
        self.dataset: Optional[Dataset] = create_local_dataset(dataset_name=self.dataset_name,
                                                               split=split,
                                                               dataset_kwargs=dataset_kwargs)
        
        if self.train_ratio is not None:
            train_idx_range = int(len(self.dataset) * self.train_ratio)
            if self.mode == "sample":
                train_idx_range = range(train_idx_range)
            elif self.mode == "traversal":
                train_idx_range = range(train_idx_range, len(self.dataset))
            self.dataset = self.dataset.select(train_idx_range)
