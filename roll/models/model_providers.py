...
def default_diffusion_module_provider(
    ...
    if model_args.model_config_kwargs["model_name"] == "wan2_2":
        from roll.pipeline.diffusion.modules.wan_module import \
            WanTrainingModule
...
def default_embedding_model_provider(
    ...
    if (
        ...
        model = HfAutoModel.from_pretrained(model_args.model_name_or_path, **init_kwargs)
...
def default_reward_model_provider(
    ...
    if (
        ...
        if model_args.model_type in ["auto_sequence_classification"]:
            ...
            from roll.models.trl_patches import (no_set_device_hook_post_init,
...
def get_extra_data_provider(model_name_or_path: str, processor=None):
    ...
    if "qwen2" in config.model_type:
        ...
        from transformers import \
            BatchFeature  # help define a object to accesss attr
