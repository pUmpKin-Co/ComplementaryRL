...
from roll.distributed.executor.worker import RankInfo, Worker
...
class Cluster:
    ...
    def __init__(
        ...
        self.worker2nodes = dict(
            zip(
                self.workers,
                ray.get([worker.get_node_ip.remote() for worker in self.workers]),
            )
        )
    ...
    def _create_workers(self):
        num_workers_per_gpu = getattr(self.worker_config, "num_workers_per_gpu", 1)
        placement_groups: List[List[Dict]] = self.resource_manager.allocate_placement_group(
            device_mapping=self.worker_config.device_mapping,
        ...
        for rank, pgs in enumerate(placement_groups):
            ...
            env_vars = {
                ...
                # Keep runtime behaviour aligned with the proven roll_backup implementation.
                # These envs are relied on by some strategies (e.g. Megatron) and tooling.
                ...
                "RAY_DEDUP_LOGS": "0",
                "REPORT_PROFILING_METRICS": os.environ.get("REPORT_PROFILING_METRICS", "1"),
                "SKIP_OLD_LOG_PROBS": os.environ.get("SKIP_OLD_LOG_PROBS", "False"),
                "NVTE_TORCH_COMPILE": os.environ.get("NVTE_TORCH_COMPILE", "1"),
            }
            ...
            if deploy_pg["gpu_rank"] is not None:
                RayUtils.update_env_vars_for_visible_devices(env_vars=env_vars, gpu_ranks=pg_zero_gpu_ranks)
            if "ROLL_LOG_DIR" in os.environ:
                env_vars["ROLL_LOG_DIR"] = os.environ["ROLL_LOG_DIR"]
            if self.worker_config.system_envs is not None:
