...
@dataclass
class ReplayBuffer:
    # all status changing should be with lock
    by_request_id: Dict[str, ExperienceItem] = field(default_factory=dict)
    request_ids_by_prompt_id: Dict[int, Set[str]] = field(
        default_factory=lambda: defaultdict(set)
    )  # for expand return sequences
    prompt_ids_by_start_step: Dict[int, Set[int]] = field(default_factory=lambda: defaultdict(set))

    # status index
    running_request_ids: Set[str] = field(default_factory=set)
    pause_request_ids: Set[str] = field(default_factory=set)

    finished_ids_by_prompt_id: Dict[int, Set[str]] = field(default_factory=lambda: defaultdict(set))
    # all requests for this prompt_id are finished, manually marked
    finished_prompt_ids_by_start_step: Dict[int, Set[int]] = field(default_factory=lambda: defaultdict(set))

    def info(self) -> str:
        return (
            f"ReplayBuffer: "
            f"by_request_id={len(self.by_request_id)}, "
            f"request_ids_by_prompt_id={len(self.request_ids_by_prompt_id)}, "
            f"running_request_ids={len(self.running_request_ids)}, "
            f"pause_request_ids={len(self.pause_request_ids)}, "
            f"finished_prompt_ids_by_start_step={sum(len(ids) for ids in self.finished_prompt_ids_by_start_step.values())}, "
            f"finished_ids_by_prompt_id={len(self.finished_ids_by_prompt_id)}"
        )
    ...
    def add_request_item(self, request_item: ExperienceItem):
        ...
        self.prompt_ids_by_start_step[request_item.sampling_start_step].add(request_item.prompt_id)
    ...
    def report_item(self, data: DataProto, is_finished: bool = True):
        ...
        if is_finished:
            self.finished_ids_by_prompt_id[item.prompt_id].add(request_id)
        else:
    ...
    def get_finished_data_by_prompt_id(self, prompt_id: int) -> List[DataProto]:
        ...
        """
        request_ids = self.finished_ids_by_prompt_id.get(prompt_id, set())
        return [self.by_request_id[request_id].data for request_id in request_ids if request_id in self.by_request_id]
    ...
    def mark_prompt_id_finished(self, prompt_id: int, num_return_sequences: int) -> list[ExperienceItem]:
        ...
        item = self.by_request_id.get(next(iter(finished_ids)))
        self.finished_prompt_ids_by_start_step[item.sampling_start_step].add(prompt_id)
        ...
        for request_id in extra_request_ids:
            extra_items.append(self.by_request_id.get(request_id))
    ...
    def pop_paused_item(self) -> Optional[ExperienceItem]:
        ...
        """
        if len(self.pause_request_ids) == 0:
            return None
        request_id = self.pause_request_ids.pop()
        item = self.by_request_id.get(request_id)
        self.running_request_ids.add(request_id)
        return item
    ...
    def get_enough_finished_prompt_ids(
        ...
        if min_step is None:
            ...
            if not (
                sum(len(p_ids) for p_ids in self.finished_prompt_ids_by_start_step.values()) >= total_prompt_num
            ):
                return None
            return list(set.union(*self.finished_prompt_ids_by_start_step.values()))[: total_prompt_num]
        required_num_by_step = {min_step: min(min_step_prompt_num, len(self.prompt_ids_by_start_step[min_step]))}
        # consume small step prompts first
        for step in sorted(list(self.finished_prompt_ids_by_start_step.keys())):
            if step <= min_step:
                continue
            last_required_prompt_num = total_prompt_num - sum(required_num_by_step.values())
            if last_required_prompt_num <= 0:
                break
            required_num_by_step[step] = min(last_required_prompt_num, len(self.prompt_ids_by_start_step[step]))
        if sum(required_num_by_step.values()) < total_prompt_num:
            # didn't send enough samples
            return None

        required_prompt_ids = []
        for step, required_num in required_num_by_step.items():
            if step in self.finished_prompt_ids_by_start_step:
                if len(self.finished_prompt_ids_by_start_step[step]) < required_num:
                    return None
                # shuffle here? since some prompt_ids may be dropped here
                required_prompt_ids.extend(list(self.finished_prompt_ids_by_start_step[step])[:required_num])
    ...
    def reset_status(self, min_step: Optional[int] = None, required_prompt_num: Optional[int] = None):
        ...
        for start_step in list(self.prompt_ids_by_start_step.keys()):
            if min_step is None or start_step < min_step:
                del_prompt_ids = self.prompt_ids_by_start_step.pop(start_step, set())
                self.finished_prompt_ids_by_start_step.pop(start_step, None)
            elif start_step == min_step:
                del_prompt_ids = self.prompt_ids_by_start_step[start_step]
                del_num = len(del_prompt_ids) - required_prompt_num
                if del_num <= 0:
                    continue
                del_prompt_ids = random.sample(list(del_prompt_ids), del_num)
                self.prompt_ids_by_start_step[start_step] -= set(del_prompt_ids)
        ...
        self.by_request_id = {
            request_id: item for request_id, item in self.by_request_id.items() if item.status != ExpStatus.DELETED
        }
        for request_id in self.running_request_ids:
            item = self.by_request_id.get(request_id)
    ...
    def is_prompt_id_finished(self, prompt_id: int) -> bool:
        for prompt_set in self.finished_prompt_ids_by_start_step.values():
            if prompt_id in prompt_set:
                return True
        return False

    def pop_prompt_id(self, prompt_id: int):
        running_items, finished_items = [], []
        request_ids = self.request_ids_by_prompt_id.pop(prompt_id)
        ...
        start_step = item.sampling_start_step
        if start_step in self.finished_prompt_ids_by_start_step:
            self.finished_prompt_ids_by_start_step[start_step].discard(prompt_id)
        if start_step in self.prompt_ids_by_start_step:
            self.prompt_ids_by_start_step[start_step].discard(prompt_id)
...
class AsyncDynamicSamplingScheduler:
    ...
    def reset_status(self):
        self.exception_queue = queue.Queue()
        if self.is_val:
            min_start_step, required_prompt_num = None, None
        else:
            min_start_step = self.global_step - math.ceil(self.pipeline_config.async_generation_ratio)
            min_step_ratio = self.pipeline_config.async_generation_ratio % 1 + max(
                0, self.init_global_step - min_start_step
            )
            required_prompt_num = round(self.batch_size * min_step_ratio) if min_step_ratio > 0 else self.batch_size
    ...
    def get_batch(self, data: DataProto, batch_size: int) -> DataProto:
        ...
        min_start_step, min_step_prompt_num = None, None
        if not self.is_val:
            min_start_step = min(
                self.global_step - math.ceil(self.pipeline_config.async_generation_ratio), self.init_global_step
            )
            min_step_ratio = self.pipeline_config.async_generation_ratio % 1
            min_step_prompt_num = round(batch_size * min_step_ratio) if min_step_ratio > 0 else batch_size
        while True:
            with self.lock:
    ...
    def pause_sampling(self, data: DataProto):
        ...
        timeout = 120
        while True:
            if len(self.replay_buffer.running_request_ids) == 0:
                break
            if time.time() - start_time > timeout:
                logger.warning(f"Timeout after {timeout}s waiting for running requests to complete. "
                             f"Remaining running requests: {len(self.replay_buffer.running_request_ids)}")
                break
            self.check_response_callback()
            time.sleep(1)
    ...
    def sending_request(self, data: DataProto):
        ...
        buffer_prompt_num = self.batch_size * min(1.0, self.pipeline_config.async_generation_ratio)
        if self.is_use_additional_prompts:
        ...
        while True:
            if self.stop_sending_requests_event.is_set():
                break
            if (
                len(self.replay_buffer.request_ids_by_prompt_id) >= buffer_prompt_num
            ):
                time.sleep(1)
                continue
            all_refs.extend(self.send_next_data_item(data))
    ...
    def report_response(self, data: DataProto):
        try:
            ...
            is_finished = self.replay_buffer.is_report_data_finished(data)
