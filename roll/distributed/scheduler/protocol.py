...
def _make_missing_non_tensor_array(size: int) -> np.ndarray:
    missing_array = np.empty(size, dtype=object)
    missing_array[:] = None
...
class DataProto:
    ...
    # Keys that are batch-level metadata, not per-sample arrays.
    _GLOBAL_NON_TENSOR_KEYS = {"triggered_interactions"}
    ...
    def __getitem__(self, item):
        ...
        # Case 3: Single integer - return DataProtoItem for backward compatibility
            ...
            for key, val in self.non_tensor_batch.items():
                if key in self._GLOBAL_NON_TENSOR_KEYS:
    ...
    def concat(
        ...
        *,
        ...
        non_tensor_batch = {}
        for k in all_non_tensor_keys:
            v = [d.non_tensor_batch.get(k) for d in data]
            # Global (batch-level) metadata should not be concatenated across ranks/chunks.
            # Otherwise, broadcasting in DP split would multiply it (e.g., 6 items * dp_size).
            if k in DataProto._GLOBAL_NON_TENSOR_KEYS:
                # NOTE:
                # - Most global keys should not be concatenated across ranks/chunks.
                # - `triggered_interactions` is a special case: each rollout shard can contribute
                #   distinct interactions (e.g., for memory-actor training). If we keep only one
                #   shard's list, the training buffer will severely underfill.
                #
                # To remain safe under DP splitting (where a global list might be broadcast into
                # each chunk), we deduplicate ONLY broadcasted copies of the *container* (same object),
                # and we do NOT deduplicate individual interactions by request_id.
                if k == "triggered_interactions":
                    merged: List[Any] = []
                    ...
                    for item in v:
                        ...
                        # Prefer treating the value as a list of interactions.
                        # For broadcasted globals, `chunk()` assigns the exact same object to each chunk,
                        # so `id(item)` dedup prevents multiplicative growth.
                        if isinstance(item, list):
                            cid = id(item)
                            if cid in seen_container_ids:
                                continue
                            seen_container_ids.add(cid)
                            merged.extend([x for x in item if x is not None])
                            continue
                        ...
                        if isinstance(item, np.ndarray):
                            try:
                                ...
                                as_list = [item]
                            merged.extend([x for x in (as_list if isinstance(as_list, list) else [as_list]) if x is not None])
                            continue

                        merged.append(item)

                    non_tensor_batch[k] = merged
                    continue
                ...
                for item in v:
                    if item is None:
                        continue
                    try:
                        if len(item) == 0:  # type: ignore[arg-type]
                            continue
                    except Exception:
                        # If it doesn't support len(), treat it as a valid value.
                        pass
                    chosen = item
                    break
                non_tensor_batch[k] = chosen if chosen is not None else v[0]
                continue

            per_sample_values = [
                item if item is not None else _make_missing_non_tensor_array(len(d))
                for d, item in zip(data, v)
            ]
            non_tensor_batch[k] = custom_np_concatenate(per_sample_values)
