...
from roll.utils.ray_utils import get_node_rank, get_visible_gpus
...
class ResourceManager:
    def __init__(self, num_gpus_per_node, num_nodes):
        ...
        nodes_maybe_used = sorted(
            nodes_maybe_used, key=lambda n: n["Resources"]["CPU"]
        )
        ...
        assert num_nodes <= ray_num_nodes, (
            f"The Ray clusters(ray_num_nodes: {ray_num_nodes}) cannot meet the "
        ...
        if self.gpu_per_node > 0:
            assert (
                self.num_gpus <= available_gpu
            ), f"num_gpus {self.num_gpus} > available_gpu {available_gpu}"
            bundles = []
            for i in range(self.num_nodes):
                node = nodes_maybe_used[i]
                node_cpu = int(node["Resources"]["CPU"])
                bundles.append(
                    {"GPU": self.gpu_per_node, "CPU": max(node_cpu / 2, 1)}
                )

            self.placement_groups = [
                ray.util.placement_group([bundle]) for bundle in bundles
            ]
            ray.get([pg.ready() for pg in self.placement_groups])
            gpu_ranks = ray.get(
                [
                    get_visible_gpus.options(
                        placement_group=pg, num_gpus=self.gpu_per_node
                    ).remote()
            ...
            self.node_ranks = ray.get(
                [
                    get_node_rank.options(placement_group=pg).remote()
                    for pg in self.placement_groups
                ]
            )
            ...
            for node_rank, placement_group in zip(
                self.node_ranks, self.placement_groups
            ):
    ...
    def allocate_placement_group(
        ...
        - num_workers_per_gpu: Number of workers sharing the same GPU (default 1 = dedicated GPU per worker)
        ...
        if device_mapping:
            if num_workers_per_gpu > 1:
                # Multiple workers share the same GPU(s)
                # device_mapping specifies which GPUs to use (can be repeated)
                # Each worker gets assigned to GPUs in round-robin fashion
                for worker_idx in range(world_size):
                    # Calculate which GPU(s) this worker should use
                    # Round-robin assignment across available GPUs
                    gpu_idx = worker_idx % len(device_mapping)
                    rank = device_mapping[gpu_idx]
                ...
                grouped_ranks = [
                    ...
                    for i in range(
                        0, len(device_mapping), num_gpus_per_worker
                    )
                ]
                ...
                allocated_pg.append(
                    [
                        dict(
                            node_rank=node_rank,
                            gpu_rank=None,
                            placement_group=self.nodes_placement_group(
                                node_rank
                            ),
                            ray_address=ray_address,
                        )
