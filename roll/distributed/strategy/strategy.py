...
from roll.utils.functionals import entropy_from_logits, get_dist_info_from_comm_plan, log_probs_from_logits
...
class TrainStrategy(InferenceStrategy):
    ...
    def setup_collective_group(self, model_update_name, comm_plan, backend="nccl"):
        ...
        # A small all_reduce for warmup.
        # Ensure CUDA device is properly set before creating CUDA tensors
        device_id = self.worker.local_rank if hasattr(self.worker, 'local_rank') else 0
