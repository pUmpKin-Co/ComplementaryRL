...
class VllmStrategy(InferenceStrategy):
    ...
    def initialize(self, model_provider):
        if getattr(self.worker, "pipeline_config", None) is not None:
            seed = self.worker.pipeline_config.seed
        else:
            seed = self.worker_config.seed
        set_seed(seed=seed)
        vllm_config = copy.deepcopy(
        ...
        if self.is_lora:
            ...
            vllm_config["load_format"] = (
                "auto"  # enables vLLM to load the base model for add_lora
            )
        ...
        if engine_mode == "sync":
            ...
            self.tokenizer = loop.run_until_complete(
        ...
        special_tokens = [
            ...
            if add_token.special
            and add_token.content not in additional_special_tokens
        ]
        ...
        logger.info(
            f"add {special_tokens} to additional_special_tokens: {self.tokenizer.additional_special_tokens}"
        )
    ...
    def embed(
        self,
        batch: DataProto,
        pool_func: Callable[[torch.Tensor, torch.Tensor], torch.Tensor],
    ):
        texts = batch.non_tensor_batch["texts"].tolist()
        micro_batch_size = batch.meta_info["micro_batch_size"]
        micro_batches = [
            texts[i : i + micro_batch_size]
            for i in range(0, len(texts), micro_batch_size)
        ]
        embeddings = []
        for data in micro_batches:
            outputs = self.model.embed(data)
    ...
    def generate(self, batch: DataProto, generation_config) -> torch.Tensor:
        ...
        attention_mask = batch.batch[
            "attention_mask"
        ]  # left-padded attention_mask

        vllm_input_args = {}
        if "multi_modal_data" in batch.non_tensor_batch:
            vllm_input_args["prompts"] = batch.non_tensor_batch[
                "multi_modal_data"
            ]
        else:
    ...
    def process_vllm_output(
        ...
        for request_output in vllm_outputs:
            ...
            output_data = DataProto(
                meta_info=self.request_metas[request_id]
            )
        ...
        while True:
            while not self.command_queue.empty():
                ...
                if command == GenerateRequestType.ADD:
                    ...
                    generation_config = batch.meta_info.get(
                        "generation_config"
                    )
                    max_new_tokens = batch.meta_info.get(
                        "max_new_tokens",
                    ...
                    if self.is_lora:
                        ...
                        lora_int_ids = list(
                            self.model.llm_engine.list_loras()
                        )
                        if len(lora_int_ids) > 0:
                            lora_int_id = lora_int_ids[0]
                            lora_requests = [
                    ...
                    self.model.abort_request(
                        request_id=list(self.request_metas.keys())
                    )
    ...
    async def async_generate(
        self, batch: DataProto, generation_config: Dict
    ) -> torch.Tensor:
        ...
        assert (
            input_ids.size(0) == 1
        ), f"async_generate: batch['input_ids'] must have exactly one batch dimension"
    ...
    def setup_collective_group(
        self, model_update_name, comm_plan, backend="nccl"
    ):
        self.model.setup_collective_group(
            comm_plan=comm_plan,
            backend=backend,
            rank_in_cluster=self.worker.rank,
        )
    ...
    def broadcast_parameter(
        ...
        self.model.broadcast_parameter(
            src_pp_rank, dtype, shape, parameter_name, is_lora
        )

    def broadcast_bucket(
        self, model_update_name, src_pp_rank, meta_infos, bucket_size
    ):
    ...
    def update_parameter(
        ...
        self.model.update_parameter(
            parameter_name, weight, ranks_in_worker, is_lora
        )

    def update_parameter_in_bucket(
        self, model_update_name, meta_infos, buffer, ranks_in_worker
    ):
        self.model.update_parameter_in_bucket(
            meta_infos, buffer, ranks_in_worker
        )
...
def gather_outputs_to_pad_tensor(
    request_outputs: List["RequestOutput"], pad_token_id, device="cuda"
) -> torch.Tensor:
...
def create_sampling_params_for_vllm(gen_kwargs):
    # 处理 stop_words，过滤掉空字符串
    stop_words = gen_kwargs.get("stop_words", None)
    if stop_words is None:
        stop_words = gen_kwargs.get("stop_strings", [])
    if stop_words:
        # 过滤掉空字符串和 None 值
        valid_stop_words = [
            word for word in stop_words if word and word.strip()
        ]
        # 如果没有有效的 stop_words，设置为 None
        stop_param = valid_stop_words if valid_stop_words else None
    else:
        stop_param = None
    print(f"[vllm stop_param]{stop_param}")
...
def compare_sampling_params(
    params1: SamplingParams, params2: SamplingParams
) -> bool:
    # 只比较采样参数的配置
    param_attrs = [
        "temperature",
