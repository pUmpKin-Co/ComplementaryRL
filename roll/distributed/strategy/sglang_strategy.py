...
class SgLangStrategy(InferenceStrategy):
    ...
    def offload_states(self, include=None, non_blocking=False):
        # TODO: use sleep level control offload, not by config
        if getattr(self.worker.pipeline_config, "async_pipeline", False):
            if self.running is not None:  # has ran
                self.model.flush_cache()
        elif include is None or OffloadStateType.model_params in include:
...
def create_sampling_params_for_sglang(gen_kwargs: dict):
    ...
    valid_stop_words = [w for w in (stop_words or []) if w and w.strip()]
