...
class MegatronInferStrategy(InferenceStrategy):
    ...
    @staticmethod
    def _align_seq_len(feature: torch.Tensor, target_seq_len: int, pad_value: int = 0) -> torch.Tensor:
        """Align a [batch, seq] tensor to target sequence length by truncate-or-pad."""
        if feature.size(1) == target_seq_len:
            return feature
        if feature.size(1) > target_seq_len:
            return feature[:, :target_seq_len]
        pad = torch.full(
            (feature.size(0), target_seq_len - feature.size(1)),
            pad_value,
            dtype=feature.dtype,
    ...
    def op_compute_log_probs(self, logits: torch.Tensor, input_ids: torch.Tensor, attention_mask: torch.Tensor):
        ...
        """
        # here attention_mask is response_mask
        #
        # Important: `logits` may be computed on a CP-sliced / truncated sequence
        # length (controlled by Megatron schedule `seq_length`). We must ensure
        ...
        labels = self._align_seq_len(labels, logits_seq_len, pad_value=0)

        log_probs = vocab_parallel_logprobs(logits, labels)
        if mpu.get_context_parallel_world_size() > 1:
            log_probs = context_parallel_gather(log_probs, parallel_dim=1)
        response_mask = self._align_seq_len(attention_mask[:, 1:], max(log_probs.size(1) - 1, 0), pad_value=0)
        log_probs = log_probs[:, :-1] * response_mask
        return log_probs
    ...
    def op_compute_entropy(self, logits: torch.Tensor, attention_mask: torch.Tensor):
        ...
        response_mask = self._align_seq_len(attention_mask[:, 1:], max(entropy.size(1) - 1, 0), pad_value=0)
        entropy = entropy[:, :-1] * response_mask
        return entropy
...
class MegatronTrainStrategy(MegatronInferStrategy, TrainStrategy):
    ...
    def initialize(self, model_provider):
        ...
        logger.info(f"dp_size: {self.worker.rank_info.dp_size}")
        logger.info(f"original max_steps: {self.worker_config.training_args.max_steps}")
        logger.info(
            f"calculated max_steps: {self.worker_config.training_args.max_steps // self.worker.rank_info.dp_size}"
        )
