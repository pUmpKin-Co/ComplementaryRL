...
from roll.utils.offload_states import (OffloadStateType, load_hf_model,
                                       offload_hf_model)
...
class HfInferStrategy(InferenceStrategy):
    ...
    def initialize(self, model_provider):
        if getattr(self.worker, "pipeline_config", None) is not None:
            ...
            set_seed(seed=self.worker_config.seed)
    ...
    def embed(self, batch: DataProto, pool_func: Callable[[torch.Tensor, torch.Tensor], torch.Tensor]):
        self.model.eval()
        batch_size = batch.batch.batch_size[0]
        micro_batch_size = batch.meta_info["micro_batch_size"]
        num_microbatches = max(batch_size // micro_batch_size, 1)
        ...
        for data in micro_batches:
            ...
            embedding = pool_func(last_hidden_state, attention_mask)
            normalized_embedding = torch.nn.functional.normalize(embedding, dim=-1, p=2)
    ...
    def update_parameter_in_bucket(self, model_update_name, meta_infos, buffer, ranks_in_worker):
        if dist.get_rank() not in ranks_in_worker:
            return
        from mcore_adapter.models.converter.convert_utils import \
            RecvBucketManager
